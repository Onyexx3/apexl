...
export default function SavingsPlans() {
  ...
  const onSubmit = (data: InsertSavingsPlan) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage member savings plans with 31-day timeline
          </p>
        </div>
        ...
        <CardContent>
          {plansLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm font-medium">No savings plans</p>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {plan.contributionsCount}/{correctMaxContributions}
                          </span>
                        </div>
      ...
      {pendingPayoutPlans.length > 0 && (
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <Clock className="w-5 h-5 text-amber-500" />
              <CardTitle className="text-lg">Pending Payout Plans</CardTitle>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <Card key={plan.id} className="flex flex-col border-amber-200 bg-amber-50/50" data-testid={`card-pending-plan-${plan.id}`}>
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between">
                        ...
                        <Badge variant="outline" className="ml-2 flex-shrink-0 bg-amber-100 text-amber-700 border-amber-300">
                          Pending Payout
                        </Badge>
                      </div>
                    </CardHeader>
                    <CardContent className="flex-1 space-y-4">
                      ...
                      <Button
                        ...
                        className="w-full mt-4"
                        asChild
                        data-testid={`button-view-pending-plan-${plan.id}`}
                      >
                        <Link href={`/plans/${plan.id}`}>View Details</Link>
                      </Button>
                    </CardContent>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Set up a new 31-day savings plan for a member. The plan will auto-close
              when 31 contributions are completed or after 62 days.
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>

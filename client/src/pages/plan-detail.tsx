...
export default function PlanDetail() {
  const [, memberParams] = useRoute("/members/:memberId/plans/:planId");
  const [, directParams] = useRoute("/plans/:planId");
  const [dialogOpen, setDialogOpen] = useState(false);
  const [overflowDialogOpen, setOverflowDialogOpen] = useState(false);
  const [overflowAmount, setOverflowAmount] = useState(0);
  const planId = memberParams?.planId || directParams?.planId;
  const memberIdFromRoute = memberParams?.memberId;
  const { toast } = useToast();

  const { data: plan, isLoading: planLoading } = useQuery<SavingsPlan & { contributions?: PlanContribution[]; member?: Member }>({
    queryKey: ["/api/savings-plans", planId],
    enabled: !!planId,
  });

  const memberId = memberIdFromRoute || plan?.memberId;
